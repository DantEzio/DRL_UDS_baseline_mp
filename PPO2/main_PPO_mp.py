...
def interact(i,ep):   
    ...
    while not done:
        ...
        at = tem_model.action_table[int(action[0].numpy())-1].tolist()
...
if Train:
    ...
    for epoch in range(model.params['training_step']):
        ...
        for _ in range(model.params['train_policy_iterations']):
            PPO.train_policy(observation_buffer, action_buffer, logprobability_buffer, advantage_buffer, model)
...
def test(model,rain,i):
    ...
    while not done:
        ...
        at=model.action_table[int(action[0].numpy())-1].tolist()
